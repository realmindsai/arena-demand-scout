...
def compute_sa2_scores(merged_geojson: dict) -> dict:
    """Score each SA2 region by child population density.

    Uses percentile ranking for children_per_sqkm (60% weight) and
    raw pop_0_4 (40% weight). Percentile ranking gives better visual
    spread across ~2,500 SA2 regions compared to min-max normalization.

    Formula:
        density_pctile = rank(density) / N              # 0-1
        pop_pctile = rank(pop) / N                      # 0-1
        raw_score = 0.6 * density_pctile + 0.4 * pop_pctile
